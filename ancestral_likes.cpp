...
void call_ancestral(chunkyT *chk,double **lk,int trim){
  ...
  for(int s=0;s<chk->nSites;s++){
	...
	likes1[offsets2[allele][0]] += probs2[0][qs]; //'homo'zygotic hit
	for(int o=1;o<4;o++){//'heterozygotic' hit{
	  likes1[offsets2[allele][o]] += probs2[1][qs];
	}
	for(int o=4;o<10;o++){//non hit
	  likes1[offsets2[allele][o]] += probs2[2][qs];
	}
