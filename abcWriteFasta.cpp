...
void abcWriteFasta::getOptions(argStruct* arguments) {
  ...
  if (doFasta) {
    if (arguments->inputtype == INPUT_BCF) {
      clean_bcf_counts = 1;
    }

    if (arguments->inputtype != INPUT_BAM && arguments->inputtype != INPUT_PILEUP && arguments->inputtype != INPUT_BCF) {
      fprintf(stderr, "Error: -doFasta only works with bam, pileup and bcf input\n");
      exit(0);
    }
