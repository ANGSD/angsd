...
void multiReader::printArg(FILE* argFile, argStruct* args) {
  ...
  fprintf(argFile, "\t-bcf\t%s\t(vcf/bcf Filename (can be bcf compressed or uncompressed))\n", fname);
...
void multiReader::getOptions(argStruct* arguments) {
  ...
  fname = angsd::getArg("-bcf", fname, arguments);
...
multiReader::multiReader(argStruct* args_arg) {
  gz = Z_NULL;
  myglf = NULL;myvcf = NULL;mpil = NULL;bglObj = NULL;bgenObj = NULL;mybcf = NULL;
  ...
  if (args->fai) {
    ...
    if (args->nams.size() == 0) {
      fprintf(stderr, "\t-> Must choose inputfile -bam/-glf/-glf3/-pileup/-i/-vcf-gl/-vcf-gp/-vcf-pl/-bcf/-glf10_text filename\n");
      exit(0);
    }
  ...
  if (args->inputtype == INPUT_BCF) {
    int argval = 0;

    bcf_tags_to_use = 0;
    argval = angsd::getArg("-doFasta", argval, args);
    if (argval) {
      if (argval == 1 || argval == 2) {
        bcf_tags_to_use |= BCF_TAG_FMT_AD;
      } else {
        fprintf(stderr, "\t-> Must supply -doFasta 1 or 2 when using -bcf files\n");
        exit(0);
      }

    }
    mybcf = new bcfReader(args->infile, NULL, bcf_tags_to_use, &args->regions);
    args->hd = mybcf->bamhdr;
    args->nInd = mybcf->hs->nsamples;
  }
